...
<div class="container">
	...
	<div class="text-right">
		<!-- 
			EL의 내장객체인 sessionScpoe를 통해 세션영역에 저장된
		...
		<c:choose>
			...
			<!-- 세션영역에 속성이 있을때, 즉 로그인 되었을때 -->
				<button class="btn btn-danget" onclick="location.href='logout.do';">
					로그아웃
				</button>
			</c:when>
			<c:otherwise>
				...
				<!-- 작성자 본인에게만 수정/삭제 버튼 보임 처리 -->
					<c:if test="${sessionScope.siteUserInfo.id eq row.id }">
						<button class="btn btn-primary"
							onclick="location.href='modify.do?idx=${row.idx}';">수정
						</button>
						
						<!-- 삭제 버튼을 누를 경우 idx값을 JS 함수로 전달한다. -->
						<button class="btn btn-danger"
							onclick="javascript:deleteRow(${row.idx});">삭제
						</button>
					</c:if>
				</div>
			</div>
